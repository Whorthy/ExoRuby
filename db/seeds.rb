...
rvm.articles.create({name: "Etape-1", content: "Creer le fichier .ruby-version avec le contenu suivant: `ruby-2.3.1`. Ce fichier permet de décrire ", published_at: Time.zone.now})
rvm.articles.create({name: "Etape-2", content: "Creer le fichier .ruby-gemset avec le contenu suivant: `mydoc`. Ce fichier permet de définir une "})
rvm.articles.create({name: "Etape-3", content: "Pour recharger l'environement il faut faire un `cd .` ou relancer la console", published_at: Time.zone.now})
